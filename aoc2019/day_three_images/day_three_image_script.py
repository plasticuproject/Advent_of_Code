...
def get_bounds(line):
    ...
    for value in line_list:
        if value[0] == 'R':
            bounds[0] += int(value[1])
        elif value[0] == 'L':
            bounds[0] -= int(value[1])
        elif value[0] == 'U':
            bounds[1] -= int(value[1])
        elif value[0] == 'D':
            bounds[1] += int(value[1])
        maxi.append(abs(max(bounds, key=abs)))
    for i in maxi:
        bounds.append(i)
...
def init_grid(grid_size):
    ...
    global position, mid_grid
    if grid_size % 2 != 0:
        grid_size += 1
    mid_grid = grid_size // 2
    position = [mid_grid, mid_grid]

    # set start point in middle of grid
    grid[mid_grid, mid_grid] = 'o'
        ...
        for step in range(int(instruction[1:])):
            ...
            try:
                ...
                if grid[position[0], position[1]] == symbols[0]:
                    crosses.append((position[0], position[1]))
            except:
                # mark wire location
                grid[position[0], position[1]] = symbols[1]
                c.append((position[0], position[1]))
...
def draw(instructions, color):
    ...
    # write wire traces to image
    img =  Image.open('challenge.png', 'r')
    d = ImageDraw.Draw(img)
    d.point(pixels[0], fill=(0,0,255))
    img.save('challenge.png')
    img =  Image.open('challenge.png', 'r')
    d = ImageDraw.Draw(img)
    d.point(pixels[1], fill=(255,0,0))
    img.save('challenge.png')


def distance(crosses):

    # write wire trace crosses to image
    img =  Image.open('challenge.png', 'r')
    d = ImageDraw.Draw(img)
    d.point(crosses, fill=(255,0,255))
    img.save('challenge.png')
    d.point(position, fill=(255,255,255))
    img.save('challenge.png')
...
def run_it(line_one, line_two):
    ...
    one = (abs(get_bounds(line_one)) + 1) * 2
    two = (abs(get_bounds(line_two)) + 1) * 2
    grid_max = max(one, two)
    ...
    # transpose image because it flipped around somehow
    img =  Image.open('challenge.png', 'r')
