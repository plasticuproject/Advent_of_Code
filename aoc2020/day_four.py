...
def get_all_fields():
    """Create list of dictionary where all fields are at least present"""
    fields = ['ecl', 'pid', 'eyr', 'hcl', 'byr', 'iyr', 'hgt']
    all_fields = []
    for i in DATA:
        for j in i:
...
def part_two():
    ...
    for i in data:
        ...
        ecl = i['ecl']
        pid = i['pid']
        if byr.isdigit and int(byr) in range(1920, 2003):
            score += 1
        if iyr.isdigit and int(iyr) in range(2010, 2021):
            score += 1
        if eyr.isdigit and int(eyr) in range(2020, 2031):
            score += 1
        if hgt.endswith('cm'):
            if hgt[:-2].isdigit and int(hgt[:-2]) in range(150, 194):
                score += 1
        if hgt.endswith('in'):
            if hgt[:-2].isdigit and int(hgt[:-2]) in range(59, 77):
                score += 1
